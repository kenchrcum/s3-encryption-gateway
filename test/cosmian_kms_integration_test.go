...
func startCosmianKMS(t *testing.T) (containerID string, endpoint string, tlsCfg *tls.Config, cleanup func(), containerName string, caCertPath string) {
	...
	// This matches the user's working example: docker run --rm -d -p 5696:5696 -p 9998:9998 --entrypoint cosmian_kms ghcr.io/cosmian/kms:5.14.1
	...
	// This matches: docker run --rm -d -p 5696:5696 -p 9998:9998 --entrypoint cosmian_kms ghcr.io/cosmian/kms:5.14.1
	...
	t.Logf("Docker command: docker run -d --name %s -p 5696:5696 -p 9998:9998 --entrypoint cosmian_kms --rm ghcr.io/cosmian/kms:5.14.1", containerName)
