...
- [ ] Add table-driven tests covering: first-byte, last-byte, suffix ranges, cross-chunk boundaries, empty ranges, out-of-bounds clamping.
- [ ] Ensure decrypter aligns and decrypts only required chunks; verify authentication tags for touched chunks.
- [ ] Validate `Content-Range`/`Content-Length` mapping from plaintext range to encrypted object sizes.
- [ ] Guarantee deterministic ETag behavior/translation; document behavior where passthrough isn’t possible.
- [ ] Optimize to minimize extra chunk reads; document worst-case overhead.
...
### Definition of Done
- [ ] All table tests pass; no regressions in `engine_test.go`
- [ ] Time-to-first-byte regression <5% vs plaintext baseline for typical ranges
- [ ] Docs updated in `docs/ENCRYPTION_DESIGN.md` (range math appendix)
